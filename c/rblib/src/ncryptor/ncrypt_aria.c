...
////////////////////////////////////////////////////////////////
////                                                        ////
//// [ ARIA Libaray Source Distributed by KISA (Modified) ] ////
////                                                        ////
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
...
/* BY(X, Y)는 BYTE4 X의 Y번째 바이트
 * BRF(T,R)은 T>>R의 하위 1바이트
 * WO(X, Y)는 BYTE array X를 BYTE4 array로 간주할 때 Y번째 BYTE4
 */

#define BY(X,Y) (((BYTE *)(&X))[Y])
#define BRF(T,R) ((BYTE)((T)>>(R)))
#define WO(X,Y) (((BYTE4 *)(X))[Y])

/* abcd의 4 BYTE로 된 BYTE4를 dcba로 변환하는 함수  */
#if defined(_MSC_VER)
/* MSC 사용 환경의 경우에는 _lrotr() 함수를
 * 이용할 수 있으므로 약간의 속도 향상이 가능하다. */
#define ReverseBYTE4(W) {						\
...
/* BYTE array를 BYTE4에 싣는 함수.  LITTLE_ENDIAN의 경우
 * 엔디안 변환 과정을 거친다. */
#ifdef _LITTLE_ENDIAN_CPU_
#define BYTE4Load(ORIG, DEST) {			\
    BYTE4 ___t;					\
    BY(___t,0)=BY(ORIG,3);			\
    BY(___t,1)=BY(ORIG,2);			\
    BY(___t,2)=BY(ORIG,1);			\
...
#define P(T0,T1,T2,T3) {					\
  ...
  * 결과적으로 BYTE4 단위로 endian을 뒤집은 결과를 준다.
...
static void Crypt(const BYTE *i, int Nr, const BYTE *rk, BYTE *o) {
  register BYTE4 t0, t1, t2, t3;
  
  BYTE4Load(WO(i,0), t0); BYTE4Load(WO(i,1), t1);
  BYTE4Load(WO(i,2), t2); BYTE4Load(WO(i,3), t3);
...
static int EncKeySetup(const BYTE *mk, BYTE *rk, int keyBits) {
  register BYTE4 t0, t1, t2, t3;
  BYTE4 w0[4], w1[4], w2[4], w3[4];
  int q, r;
  
  BYTE4Load(WO(mk,0), w0[0]); BYTE4Load(WO(mk,1), w0[1]);
  BYTE4Load(WO(mk,2), w0[2]); BYTE4Load(WO(mk,3), w0[3]);
  ...
  FO;
  if (keyBits > 128) {
    BYTE4Load(WO(mk,4), w1[0]);
    BYTE4Load(WO(mk,5), w1[1]);
    if (keyBits > 192) {
      BYTE4Load(WO(mk,6), w1[2]);
      BYTE4Load(WO(mk,7), w1[3]);
    } else {
      w1[2]=w1[3]=0;
    }
  } else {
...
static int DecKeySetup(const BYTE *mk, BYTE *rk, int keyBits) {
  ...
  for (; a<z; a+=4, z-=4) {
    BYTE4M1(a[0],t0); BYTE4M1(a[1],t1); BYTE4M1(a[2],t2); BYTE4M1(a[3],t3);
    MM(t0,t1,t2,t3) P(t0,t1,t2,t3) MM(t0,t1,t2,t3)
      s0=t0; s1=t1; s2=t2; s3=t3;
    BYTE4M1(z[0],t0); BYTE4M1(z[1],t1); BYTE4M1(z[2],t2); BYTE4M1(z[3],t3);
    MM(t0,t1,t2,t3) P(t0,t1,t2,t3) MM(t0,t1,t2,t3)
...
encrypted ciphertext.\n");
  int SZKEY = 192;
  printf("plaintext : "); printBlock(p); printf("\n");
  Crypt(p, EncKeySetup(mk, rk, SZKEY), rk,c);
  printf("ciphertext: "); printBlock(c); printf("\n");
  DecKeySetup(mk, rk, SZKEY);
  Crypt(c, DecKeySetup(mk, rk, SZKEY), rk, p);
...
static int kisa_aria_ecb_main(int argc, char **argv) {
  ARIA_test();
  return 0;
}
...
void ARIA_KeySched(BYTE *UserKey, BYTE4 UserKeyLen, BYTE *RoundKey, int isDecKey)
{
    if ( isDecKey == 1 )
    {
        DecKeySetup(UserKey, RoundKey, UserKeyLen * 8);
    }
    else
    {
        EncKeySetup(UserKey, RoundKey, UserKeyLen * 8);
    }
}
...
////////////////////////////////////////////////////////////////
////                                                        ////
//// [ Ncrypt ARIA Interface Source ]                       ////
////                                                        ////
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////

#define N_ROUNDS_MAX 16 // 12 // 14 // 16
#define SZ_BLOCK 16
#define SZ_KEY_MAX 32 // 16 // 24 // 32
#define SZ_ROUND_KEY_MAX (SZ_BLOCK * (N_ROUNDS_MAX + 1))

#define GET_N_ROUNDS(eRound) (12 + (eRound) + (eRound)) // 12, 14, 16rounds
#define GET_SZKEY_NEED(eRound, nRound) ((nRounds) + 4) + ((eRounds) * 6)

static BYTE gAB_LAST_KEY[SZ_KEY_MAX];
static BYTE gAB_LAST_ENC_ROUND_KEY[SZ_ROUND_KEY_MAX];
static BYTE gAB_LAST_DEC_ROUND_KEY[SZ_ROUND_KEY_MAX];

static void EnDecrypt_ARIA(BYTE *pbBlock, int szBlock, BYTE *pbRoundKey, int szRoundKey, void *pstArgs)
{
	BYTE 				rtBuf[szBlock];
	NCRYPT_ARIA_ROUNDS	eRounds = ((NCRYPT_ALGO_ARIA_ARGS *)pstArgs)->eRounds;
    int					nRounds = GET_N_ROUNDS(eRounds);

    Crypt(pbBlock, nRounds, pbRoundKey, rtBuf);
...
{
	int nRounds   = GET_N_ROUNDS(eRounds);
	int szKeyNeed = GET_SZKEY_NEED(eRounds, nRounds);

    if (pbKey == NULL || szKey < szKeyNeed)
    {
        fprintf(stderr, "pbKey == NULL || szKey < %d (pbKey:0x%p, szKey:%d)\n", szKeyNeed, pbKey, szKey);
        return -1;
    }

    ARIA_KeySched(pbKey, szKey, gAB_LAST_ENC_ROUND_KEY, 0);
    ARIA_KeySched(pbKey, szKey, gAB_LAST_DEC_ROUND_KEY, 1);
	return 0;
}
...
int Ncrypt_ARIA(NCRYPT_ALGO_ARIA_ARGS *pstArgs)
{
    ...
    /* - Use 16/24/32byte key(each 12/14/16rounds), 208/240/272byte round key,
    /*   input 16byte stream block, output 16byte block(NULL padding) */
    static int			IsInitalized = 0;
	NCRYPT_ARIA_ROUNDS	eRounds = pstArgs->eRounds;
	int					nRounds = GET_N_ROUNDS(eRounds);			// 12, 14, 16 rounds
	int					szKey = pstArgs->stArgs.szKey;				// 16, 24, 32 byte
	int					szKeyNeed = GET_SZKEY_NEED(eRounds, nRounds);
	int					szRoundKeyNeed = SZ_BLOCK * (nRounds + 1);	// 208, 240, 272 byte
	BYTE				*pEnDeRoundKey = NULL;
	int					rtVal;
	...
	if (eRounds < 0 || eRounds >= NCRYPT_ARIA_ROUND_MAX)
	{
		fprintf(stderr, "eRounds < 0 || eRounds >= NCRYPT_ARIA_ROUND_MAX. (eRound:%d)\n", eRounds);
		return -1;
	}

	if (szKey != szKeyNeed) // 12round -> 16byte szKey, 14 -> 24, 16 -> 32
	{
		fprintf(stderr, "Key size error. (szKey:%d != szKeyNeed:%d)\n", szKey, szKeyNeed);
		return -1;
	}

    if (IsInitalized == 0)
    {
        if ((rtVal = Ncrypt_ARIA_Init(eRounds, pstArgs->stArgs.pbKey, szKey)) < 0)
        {
            fprintf(stderr, "Ncrypt_ARIA_Init() error. (return:%d)\n", rtVal);
            return -1;
        }
	...
	pEnDeRoundKey = pstArgs->stArgs.isEncrypt == 1 ? gAB_LAST_ENC_ROUND_KEY : gAB_LAST_DEC_ROUND_KEY;

    if ((rtVal = Ncryption(&pstArgs->stArgs, SZ_BLOCK, pEnDeRoundKey, szRoundKeyNeed, EnDecrypt_ARIA)) < 0)
    {
