...
static void Blowfish_Encrypt(BLOWFISH_CTX *ctx, u32 *xl, u32 *xr){
  u32  Xl;
  u32  Xr;
  u32  temp;
  short       i;
  ...
  Xr = Xr ^ ctx->P[N_ROUNDS];
  Xl = Xl ^ ctx->P[N_ROUNDS + 1];
...
static void Blowfish_Decrypt(BLOWFISH_CTX *ctx, u32 *xl, u32 *xr){
  u32  Xl;
  u32  Xr;
  u32  temp;
  short       i;
...
////////////////////////////////////////////////////////////////
////                                                        ////
//// [ Ncrypt BLOWFISH Interface Source ]                   ////
////                                                        ////
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
*/
...
#define SZ_BLOCK 8
#define SZ_KEY_MAX 56 // 4~56 bytes

static BLOWFISH_CTX gST_CTX;

static void EnDecrypt_BLOWFISH(BYTE *pbBlock, int szBlock, BYTE *pbRoundKey, int szRoundKey, void *pstArgs)
{
    NCRYPT_ALGO_BLOWFISH_ARGS *pstBF_Args = (NCRYPT_ALGO_BLOWFISH_ARGS *)pstArgs;
    BYTE4 b4Data_L, b4Data_R;
                                        /* [01 02 03 04 05 06 07 08] (8byte) => [04 03 02 01] [08 07 06 05] (little-endian 8byte) */
    memcpy(&b4Data_L, &pbBlock[0], 4);  /* [04 03 02 01] => [01 02 03 04] */
    memcpy(&b4Data_R, &pbBlock[4], 4);  /* [08 07 06 05] => [05 06 07 08] */

    b4Data_L = htonl(b4Data_L);
    b4Data_R = htonl(b4Data_R);

    if (pstBF_Args->stArgs.isEncrypt == 1)
    {
        Blowfish_Encrypt(&gST_CTX, &b4Data_L, &b4Data_R);
    }
    else
    {
        Blowfish_Decrypt(&gST_CTX, &b4Data_L, &b4Data_R);
    }

    b4Data_L = ntohl(b4Data_L);
    b4Data_R = ntohl(b4Data_R);

    memcpy(&pbBlock[0], &b4Data_L, 4);
    memcpy(&pbBlock[4], &b4Data_R, 4);
}

static int Ncrypt_BLOWFISH_Init(BYTE *pbKey, int szKey)
{
    if (pbKey == NULL || (szKey < 4 || szKey > SZ_KEY_MAX))
    {
        fprintf(stderr, "pbKey == NULL || (szKey < 4 || szKey > %d) (pbKey:0x%p, szKey:%d)\n", SZ_KEY_MAX, pbKey, szKey);
        return -1;
    }

    Blowfish_Init(&gST_CTX, pbKey, szKey);
	return 0;
}

int Ncrypt_BLOWFISH(NCRYPT_ALGO_BLOWFISH_ARGS *pstArgs)
{
    /* Blowfish 64bit(8byte) fixed-size block en/decrypt algorithm */
    /* - Use 4~56byte key, input 8byte stream block, output 8byte stream block(NULL padding) */
    int rtVal;
    ...
    if ((rtVal = Ncrypt_BLOWFISH_Init(pstArgs->stArgs.pbKey, pstArgs->stArgs.szKey)) < 0)
    {
        fprintf(stderr, "Ncrypt_BLOWFISH_Init() error. (return:%d)\n", rtVal);
        return -1;
    }

    if ((rtVal = Ncryption(&pstArgs->stArgs, SZ_BLOCK, pstArgs->stArgs.pbKey, SZ_KEY_MAX, EnDecrypt_BLOWFISH)) < 0)
    {
