...
////////////////////////////////////////////////////////////////
////                                                        ////
//// [ SEED Libaray Source Distributed by KISA (Modified) ] ////
////                                                        ////
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
...
//     //#if defined(__alpha)
//         typedef unsigned int        BYTE4;		// unsigned 4-byte data type
//         typedef unsigned short      WORD;		// unsigned 2-byte data type
//     //#else
//     //    typedef unsigned long int   BYTE4;		// unsigned 4-byte data type
...
#define EndianChange(dwS)                       \
    ( (ROTL((dwS),  8) & (BYTE4)0x00ff00ff) |   \
      (ROTL((dwS), 24) & (BYTE4)0xff00ff00) )


/*************************** Function Declarations ****************************/

static void SEED_Encrypt(		/* encryption function */
		BYTE *pbData, 				// [in,out]	data to be encrypted
		BYTE4 *pb4RoundKey			// [in]			round keys for encryption
		);
    
static void SEED_Decrypt(		/* decryption function */
		BYTE *pbData, 				// [in,out]	data to be decrypted
		BYTE4 *pb4RoundKey			// [in]			round keys for decryption
		);
    
static void SEED_KeySchedKey(		/* key scheduling function */
		BYTE4 *pb4RoundKey, 		// [out]		round keys for encryption or decryption
...
static void SEED_Encrypt (
		...
		BYTE4 *pb4RoundKey)			// [in]			round keys for encryption
{
	BYTE4 L0, L1, R0, R1;		// Iuput/output values at each rounds
	BYTE4 T0, T1;				// Temporary variables for round function F
	BYTE4 *K = pb4RoundKey;		// Pointer of round keys
...
// Because SEED use little endian order in default
    ...
    SEED_KeySched(L0, L1, R0, R1, K   ); 	// Round  1 (0)
    SEED_KeySched(R0, R1, L0, L1, K+ 2); 	// Round  2 (8)
    SEED_KeySched(L0, L1, R0, R1, K+ 4); 	// Round  3 (16)
    SEED_KeySched(R0, R1, L0, L1, K+ 6); 	// Round  4 (24)
    SEED_KeySched(L0, L1, R0, R1, K+ 8); 	// Round  5 (32)
    SEED_KeySched(R0, R1, L0, L1, K+10); 	// Round  6 (40)
    SEED_KeySched(L0, L1, R0, R1, K+12); 	// Round  7 (48)
    SEED_KeySched(R0, R1, L0, L1, K+14); 	// Round  8 (56)
    SEED_KeySched(L0, L1, R0, R1, K+16); 	// Round  9 (64)
    SEED_KeySched(R0, R1, L0, L1, K+18); 	// Round 10 (72)
    SEED_KeySched(L0, L1, R0, R1, K+20); 	// Round 11 (80)
    SEED_KeySched(R0, R1, L0, L1, K+22); 	// Round 12 (88)
    SEED_KeySched(L0, L1, R0, R1, K+24); 	// Round 13 (96)
    SEED_KeySched(R0, R1, L0, L1, K+26); 	// Round 14 (104)
    SEED_KeySched(L0, L1, R0, R1, K+28); 	// Round 15 (112)
    SEED_KeySched(R0, R1, L0, L1, K+30); 	// Round 16 (120)
...
static void SEED_KeySchedKey(
		BYTE4 *pb4RoundKey,			// [out]	round keys for encryption or decryption
		BYTE *pbUserKey)			// [in]		secret user key
{
	BYTE4 A, B, C, D;				// Iuput/output values at each rounds
	BYTE4 T0, T1;					// Temporary variable
	BYTE4 *K = pb4RoundKey;			// Pointer of round keys
...
static void kisa_seed_ecb_main()
{
	BYTE4 pb4RoundKey[32];														// Round keys for encryption or decryption
...
// Print round keys at round i
	printf ("\n\nRound Key  : \n");
	for (i=0;i<16;i++) {
		printf("K%2d,0 : %08X\t", i+1, pb4RoundKey[2*i]);
		printf("K%2d,1 : %08X\n", i+1, pb4RoundKey[2*i+1]);
	}
}
...
static void Encrypt_SEED(BYTE *pbBlock, int szBlock, BYTE *pbRoundKey, int szRoundKey, void *pstArgs)
{
    SEED_Encrypt(pbBlock, (BYTE4 *)pbRoundKey);
}

static void Decrypt_SEED(BYTE *pbBlock, int szBlock, BYTE *pbRoundKey, int szRoundKey, void *pstArgs)
{
    SEED_Decrypt(pbBlock, (BYTE4 *)pbRoundKey);
}
...
int Ncrypt_SEED_Init(BYTE *pbKey, int szKey)
{
    ...
    SEED_KeySchedKey((BYTE4 *)gAB_LAST_ROUND_KEY, gAB_LAST_KEY);
	return 0;
}
...
int Ncrypt_SEED(NCRYPT_ALGO_SEED_ARGS *pstArgs)
{
    ...
    if (IsInitalized == 0)
    {
        if ((rtVal = Ncrypt_SEED_Init(pstArgs->stArgs.pbKey, pstArgs->stArgs.szKey)) < 0)
        {
            fprintf(stderr, "Ncrypt_SEED_Init() error. (return:%d)\n", rtVal);
            return -1;
        }
    ...
    pAlgoFunc = pstArgs->stArgs.isEncrypt == 1 ? Encrypt_SEED : Decrypt_SEED;
